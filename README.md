...
| OPR-R28-NS | Pod Security Standard Baseline profile or stricter not enforced for Operator namespace | The Operator's namespace is currently configured without a 'baseline' or 'restricted' Pod Security Standards (PSS) profile enforced. This permits the deployment of overly privileged pods, substantially increasing the risk of container escape and unauthorised access to the underlying host node. Consequently, a compromised operator could be exploited by an attacker to elevate privileges and jeopardise the security of the wider cluster environment. | High |
